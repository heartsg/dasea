...
// For configuration purpose we still use golang's flag library, which is enough.
// alecthomas/kingpin is too complex and it has not only flags but also commands
// and arguments, so it's not clear how these will be integrated into configuration
// loader.

//Unlike environment loader, which prepend struct name to front by default, for flag,
//each field is not prepended with struct name (the struct normally used with command).

//And for flatten style, even nested struct name will not be prepended.
...
type FlagLoader struct {
	...
	// Flag tag value, same as Env tag in Environment loader, default value is "flag"
	FlagTagName string
}
